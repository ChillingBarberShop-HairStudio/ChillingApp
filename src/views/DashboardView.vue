...
const emptyMetrics = (date: string, scope: ReportScope): DashboardMetrics => ({ date, scope, revenue: 0, bookingCount: 0, invoiceCount: 0, expenses: 0, profit: 0, staffCommission: 0, ownerCommission: 0, cash: { count: 0, amount: 0 }, bankTransfer: { count: 0, amount: 0 }, monthlyRevenue: [], monthlyExpenses: [], staffRank: [], customerRank: [], serviceRank: [] })
const metrics = ref<DashboardMetrics>(emptyMetrics(selectedDate.value, selectedScope.value))
...
async function load() {
  if (!isSupabaseConfigured) { error.value = 'Supabase is not configured.'; return }
...
<template>
  ...
  <p v-if="!isSupabaseConfigured" class="error-banner">Chưa có cấu hình Supabase. Dashboard không hiển thị dữ liệu minh họa.</p><p v-if="error" class="error-banner">{{ error }}</p>
  ...
  <div class="monthly-chart-grid"><RevenueChart :data="metrics.monthlyRevenue" /><RevenueChart :data="metrics.monthlyExpenses" title="Chi tiêu theo tháng" kicker="Phân tích thu chi" column-label="Cột chi tiêu" line-label="Đường biến động chi" line-color="#c45b5b" /></div>
