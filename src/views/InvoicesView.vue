...
function shareInvoice(invoice: Invoice) {
  const text = `Hóa đơn ${invoice.invoice_no} - Chilling Barber Shop - ${money(invoice.total_amount)}`
...
<template>
  <div class="page-intro compact"><div><p class="eyebrow">Dữ liệu kế toán</p><h2>Hóa đơn thanh toán</h2><p>Hóa đơn được lưu với số tự sinh, có thể xuất file PDF, in hoặc chia sẻ nội dung đến khách hàng.</p></div></div>
  <div class="invoices-layout">
    ...
    <aside v-if="activeInvoice" id="invoice-document" class="invoice-document print-document"><div class="receipt-header"><img src="/logo.PNG" alt="Chilling Barber Shop" /><h3>CHILLING BARBER SHOP</h3><p>Đường D13-5A, KCN Bàu Bàng, Bình Dương<br />Hotline: 032 796 9930</p></div><div class="receipt-title"><span>HÓA ĐƠN</span><code>{{ activeInvoice.invoice_no }}</code></div><p><b>Ngày xuất hóa đơn:</b> {{ new Date(activeInvoice.paid_at).toLocaleString('vi-VN') }}<br /><b>Khách hàng:</b> {{ activeInvoice.customer_name }}<br /><b>Số điện thoại:</b> {{ activeInvoice.customer_phone }}</p><div class="receipt-lines"><div v-for="line in activeInvoice.invoice_lines" :key="line.service_name"><span>{{ line.service_name }}<small>{{ line.staff_name }} - {{ line.quantity }} x {{ money(line.unit_price) }}</small></span><b>{{ money(line.quantity * line.unit_price) }}</b></div></div><div class="receipt-breakdown"><div><span>Tạm tính</span><strong>{{ money(activeInvoice.subtotal) }}</strong></div><div v-if="activeInvoice.discount_amount"><span>Giảm giá{{ activeInvoice.discount_percent ? ` (${activeInvoice.discount_percent}%)` : '' }}</span><strong>-{{ money(activeInvoice.discount_amount) }}</strong></div></div><div class="receipt-total"><span>Tổng thanh toán</span><strong>{{ money(activeInvoice.total_amount) }}</strong></div><p class="receipt-thanks">Cảm ơn quý khách đã tin tưởng Chilling Barber Shop.<br />Hẹn gặp lại anh/chị!</p><div class="receipt-actions print-hide" data-pdf-exclude><button class="secondary-button" :disabled="exportingPdf" @click="downloadPdf(activeInvoice)"><FileDown :size="17" /> {{ exportingPdf ? 'Đang tạo PDF...' : 'Tải PDF' }}</button><button class="secondary-button" @click="printInvoice(activeInvoice)"><ReceiptText :size="17" /> In hóa đơn</button></div></aside>
  </div>
</template>
