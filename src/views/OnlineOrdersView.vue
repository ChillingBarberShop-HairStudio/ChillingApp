...
<template>
  <div class="page-intro"><div><p class="eyebrow">Luồng online</p><h2>Đơn đặt lịch từ landing page</h2><p>Đơn chờ phục vụ sẽ được chuyển sang Thanh toán sau khi hoàn thành. Giá và nhân viên đã được chốt từ dịch vụ.</p></div></div><p v-if="error" class="error-banner">{{ error }}</p><div class="order-grid"><article v-for="order in orders" :key="order.id" class="order-card"><div class="order-card-top"><span class="status-chip" :class="`status-${order.status}`">{{ labels[order.status] }}</span><code>{{ order.booking_code }}</code></div><h3>{{ order.customer_name }}</h3><p>{{ order.customer_phone }} · {{ new Date(order.appointment_date).toLocaleDateString('vi-VN') }} lúc {{ order.time_slot.slice(0, 5) }}</p><ul><li v-for="service in order.booking_services" :key="service.service_name"><span>{{ service.service_name }}</span><strong>{{ service.staff_name }}</strong></li></ul><div class="order-card-bottom"><strong>{{ order.total_amount.toLocaleString('vi-VN') }}đ</strong><div><button v-if="order.status === 'waiting'" class="secondary-button" @click="change(order, 'serving')"><Play :size="16" /> Nhận phục vụ</button><button v-else-if="order.status === 'serving'" class="primary-button" @click="change(order, 'completed')"><CheckCheck :size="16" /> Chuyển thanh toán</button><span v-else-if="order.status === 'completed'" class="completed-label"><Clock3 :size="16" /> Đã hoàn thành</span><button v-if="order.status !== 'completed'" class="icon-text-button danger" @click="change(order, 'cancelled')"><XCircle :size="16" /> Hủy</button></div></div></article></div>
</template>
